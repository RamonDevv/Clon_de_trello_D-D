...
function saveTasks() {
  const tasks = {
    todo: Array.from(To_Do_list.children).map(li => li.textContent.replace("✕", "").trim()),
    inProgress: Array.from(In_Progress_list.children).map(li => li.textContent.replace("✕", "").trim()),
    completed: Array.from(Completed_list.children).map(li => li.textContent.replace("✕", "").trim())
  };
...
// STEP 4: Helper function to create a task item with delete button
function createTaskItem(text) {
  const li = document.createElement("li");
  li.innerHTML = `${text} <button class="delete-btn" onclick="deleteTask(this)">✕</button>`;
  return li;
}
